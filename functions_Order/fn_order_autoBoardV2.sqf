...
params ["_unitsToBoard", "_extractPos"]; 

systemChat format ["running autoboard V2 on %1", _unitsToBoard];
...
_numInj = count _unitsToBoard;

_name = str _extractPos;
systemChat format ["Creating Extract Marker: %1", _name];
...
// order boarding here 
{
	_x assignAsCargo _chosenHeli;
	systemChat format ["%1 assigned to %2", _x, _chosenHeli];
	[_x] orderGetIn true;
	[_chosenHeli, _x, 0.4, "medicalZone", [9220.78,6243.92,0]] spawn RGGo_fnc_order_autoUnboard;
	sleep 0.2;
