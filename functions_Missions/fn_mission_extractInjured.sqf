...
params ["_extractPos", "_cycleLimit", "_lzRef", "_missionType", "_raptor"];
...
while {_proxCheck} do {
	...
	if (_cnt == 0) then {
		...
		// systemChat format ["%1, %2, %3, %4", _lzRef, _missionType, _numInj, _raptor];
		[_lzRef, _missionType, _numInj, _raptor] call RGGm_fnc_mission_announcer;
	} else {
...
while {_checkCycle} do {	
	...
	_cycle = _cycle + 1;
	{
		if ((_extractPos distance (getPos _x)) < 1600) exitWith {
			...
			[_extractPos, _numInj, _raptor] spawn RGGs_fnc_spawn_boardingInjured;
