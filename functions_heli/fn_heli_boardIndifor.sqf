...
// format ["DEBUG: Pickup System / CANBOARD: %1 %2", _pilot, _heli] remoteExec ["systemChat", 0]; 
...
if (_freeCargoPositions == 0) then {	
	if !(isNull objectParent _pilot) then {
		...
		// TELEX message system ---------------------------------------------------------------------------------------------
		if (RGG_telex == false) then {
			// ok to telex 
			RGG_telex = true; // hold other telexes
			...
			sleep 10;
			RGG_telex = false; // release to other telexes 
		} else {
			_chk = true;
			while {_chk == true} do {
				if (RGG_telex == false) then {
	...
	// format ["DEBUG: Pickup System / RGG_availableIndifor: %1", RGG_availableIndifor] remoteExec ["systemChat", 0]; 
		...
		for "_i" from 1 to _freeCargoPositions do {
			if ((RGG_availableIndifor > 0)) then {
				...
				_classes = selectRandom [_classes2, _classes3];
